...
NetHack 3.2.2 is a new enhancement to the dungeon exploration game NetHack.
...
NetHack 3.2.2 er siste utvidelse til NetHack, et tøm og røm eventyr spill.
Det er basert på spill som Rouge og Hack, og er etterfølgeren til versjon
3.0 og 3.1 av NetHack.

Denne utgaven er kopilert støtte for følgende utvidelser:
X11, glibc 2.1 og ncurses.

%description -l pl
NetHack 3.2.2 -- Przygoda w Labiryntach Gro¼by.

NetHack 3.2.2 jest przygodow± gr±, której akcja toczy siê w podziemnych 
labiryntach. Wywodzi siê ze starszych gier, Rouge i Hack, i zawiera wiele
nowych rozszerzeñ w stosunku do poprzednich wersji 3.0 i 3.1.
...
install -d $RPM_BUILD_ROOT%{_datadir}/{games/nethack,fonts/misc} \
	$RPM_BUILD_ROOT{%{_mandir}/man6,/usr/X11R6/lib/X11/app-defaults} \
...
install util/recover	$RPM_BUILD_ROOT/usr/share/games/nethack
install util/*_comp	$RPM_BUILD_ROOT/usr/share/games/nethack

install win/X11/NetHack.ad \
	$RPM_BUILD_ROOT/usr/X11R6/lib/X11/app-defaults/NetHack

(cd win/X11 ;
/usr/X11R6/bin/bdftopcf < nh10.bdf > nh10.pcf
/usr/X11R6/bin/bdftopcf < ibm.bdf > ibm.pcf
install nh10.pcf ibm.pcf $RPM_BUILD_ROOT/usr/share/fonts/misc
)

gzip -9nf $RPM_BUILD_ROOT%{_mandir}/man6/* \
	$RPM_BUILD_ROOT/usr/share/fonts/misc/*.pcf \
...
%attr(0755,root,root) /usr/games/nethack
